                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                    <div class="grid grid-cols-4 gap-4">
                        ...
                        <div class="col-span-2">
                            ...
                            <select name="NRP" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1 bg-neutral-200" disabled>
                                @foreach ($mhsw as $data)
                                <option value="{{ $data['NRP'] }}"
                                @if ($data['NRP'] == $nilai['NRP'])
                                    selected
                                @endif
                                >{{ $data['nama'] }}</option>
                                @endforeach
                        ...
                        <div class="col-span-2">
                            ...
                            <select name="id_praktikum" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm w-full mt-1 bg-neutral-200" disabled>
                                @foreach ($praktikum as $data)
                                <option value="{{ $data['id_praktikum'] }}"
                                @if ($data['id_praktikum'] == $nilai['id_praktikum'])
                                    selected
                                @endif
                                >{{ $data['id_praktikum'] }}</option>
                                @endforeach
                            </select>
                            <x-input-error :messages="$errors->get('id_praktikum')" class="mt-2" />
                        </div>
                        ...
                        <div class="col-span-2">
                            ...
                            <x-text-input id="nilai" class="mt-1 w-full" type="number" name="nilai" :value="$nilai['nilai']" required autofocus autocomplete="name" />
