                ...
                <table class="w-full overflow-hidden rounded-lg">
                    <thead class="bg-slate-100">
                        <tr class="text-slate-600">
                            <th class="py-4 bg-sky-400 text-white">Kode Matkul</th>
                            <th class="py-4">Nama</th>
                            <th class="py-4">Fakultas</th>
                    ...
                    <tbody class="text-center">
                        ...
                        <tr class="border-b-2 border-slate-100 text-slate-800">
                            <td class="py-4 bg-sky-100">{{ $data['id_matkul'] }}</td>
                            <td class="py-4">{{ $data['nama'] }}</td>
                            <td class="py-4">{{ $data['fakultas'] }}</td>
                            <td class="py-4">{{ $data['semester'] }}</td>
                            <td class="py-4">{{ $data['sks'] }}</td>
                            <!-- Add other columns as needed -->
                            <td>
                                <div class="flex flex-row justify-center gap-4">

                                    <a href="{{ route('mk.edit', $data['id_matkul']) }}">
                                        <x-secondary-button>
                                            {{ __('Edit') }}
                                        </x-secondary-button>
                                    </a>
                                    ...
                                    <x-danger-button
                                        x-data=""
                                        x-on:click.prevent="$dispatch('open-modal', 'confirm-data-deletion-{{ $data['id_matkul'] }}')">{{ __('Delete Data') }}</x-danger-button>
                                    <x-modal name="confirm-data-deletion-{{ $data['id_matkul'] }}" :show="$errors->userDeletion->isNotEmpty()" focusable>
                                        <form action="{{ route('mk.destroy', $data['id_matkul']) }}" method="POST" class="p-6">
                                            @csrf
                                            @method('DELETE')
